...
<div class="news-detail">
	...
	<?if($arParams["DISPLAY_NAME"]!="N" && $arResult["NAME"]):?>
		<h3 class="art-header text-center"><?=$arResult["PROPERTIES"][GetMessage("TITLE")]["VALUE"]?></h3>
	<?endif;?>
	...
	<?if($arProp['UDK']['VALUE'] != null):?>
		<h3 class="art-udc text-left journhead"><?=GetMessage('UDK')?>: <?$arProp['UDK']['VALUE']?></h3>
	<?endif?>
		...
		<?if($arProp['PUBLTYPE']['VALUE'] != null):?>
			<div class="sci-prop">
				...
				while ($ob2 = $res->GetNextElement()):
					...
					<b class="sci-prop-color-green"><?=GetMessage('PUBLTYPE_STR')?>:</b> <a href="<?=$arFields2['DETAIL_PAGE_URL']?>"><?=$arProp2[GetMessage('TITLE')]['VALUE']?></a>
				<?endwhile;?>
			</div>
		<?endif;?>
		<?if($arProp['isOMI']['VALUE'] != null):?>
			<div class="sci-prop">
				<b class="sci-prop-color-green"><?=GetMessage('IS_OMI_STR')?>:</b> <?=GetMessage('IS_OMI_STR_VAL')?>
			</div>"
		<?endif;?>
		<?if ($arProp['KEYWORDS']['VALUE']!=null):?>
			<div class="sci-prop">
				<b class="sci-prop-color-green"><?=GetMessage('KEYWORDS')?>:</b> 
				<?$i=0;
		...
		<?if($arProp['JOURNAL']['VALUE'] != null):?>
			<div class="sci-prop">
				...
				while ($ob3 = $res->GetNextElement()):
					...
					<?=GetMessage('JOURNAL')?>: <a href="<?$arFields3['DETAIL_PAGE_URL']?>"><?=$arProp3[GetMessage('TITLE')]['VALUE']?></a>
				<?endwhile;?>
			</div>
		<?endif;?>
		<?if($arProp['PUBLDATE']['VALUE'] != null):?>
			<div class="sci-prop">
				<b class="sci-prop-color-green"><?=GetMessage('PUBL_DATE')?>:</b> <?=$arProp['PUBLDATE']['VALUE']?>
			</div>
		<?endif;?>
		<?if($arProp[GetMessage('BIBDATA')]['VALUE'] != null):?>
			<div class="sci-prop">
				<b class="sci-prop-color-green"><?=$arProp[GetMessage('BIBDATA')]['NAME']?>:</b> <?=$arProp[GetMessage('BIBDATA')]['VALUE']?>
			</div>
		<?endif;?>
	</div>
	<?if($arProp['FULLTEXT']['VALUE'] != null):?>
		<div class="sci-prop text-center">
			<a class="btn btn-success btn-lg" href="<?=$arProp['FULLTEXT']['VALUE']?>"><span class="glyphicon glyphicon-book"></span> <?=GetMessage('FULL_TEXT')?></a>
		</div>
	<?endif;?>
