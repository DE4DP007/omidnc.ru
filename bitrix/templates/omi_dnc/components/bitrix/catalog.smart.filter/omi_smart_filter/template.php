							...
							<div class="card-container" style="background-color: #f5f5f5;">
								...
								switch ($arItem["DISPLAY_TYPE"]):
									...
									case "K"://RADIO_BUTTONS?>
										...
										<?foreach($arItem["VALUES"] as $val => $ar):?>
											<div class="radio">
												<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx-filter-param-label" for="<?=$ar["CONTROL_ID"]?>">
													<span class="bx-filter-input-checkbox <?=$ar["DISABLED"] ? 'disabled': ''?>">
														<input
															type="radio"
															value="<?=$ar["HTML_VALUE_ALT"]?>"
															name="<?=$ar["CONTROL_NAME_ALT"]?>"
									...
									default://CHECKBOXES?>
										...
										<?foreach($arItem["VALUES"] as $val => $ar):?>
											<div class="checkbox card-element">
												<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx-filter-param-label <?=$ar["DISABLED"] ? 'disabled': ''?> omi-label" for="<?=$ar["CONTROL_ID"]?>">
													<span class="bx-filter-input-checkbox">
														<input
															class="hide"
															type="checkbox"
				...
				<div class="col-xs-12 bx-filter-button-box">
					<div class="bx-filter-block">
						<div class="bx-filter-parameters-box-container" style="margin-top: 20px">
							...
							/>
							<div class="bx-filter-popup-result <?if ($arParams["FILTER_VIEW_MODE"] == "VERTICAL"):?> <?=$arParams["POPUP_POSITION"]?><?endif;?>" id="modef" <?if(!isset($arResult["ELEMENT_COUNT"])) echo 'style="display:none"';?> style="display: inline-block;">
